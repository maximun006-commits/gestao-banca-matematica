...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-slate-200 font-sans p-4 md:p-8 relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-8 bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                 <button onClick={() => processarEntrada('V')} className="flex-1 py-6 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-xl transition-all flex flex-col items-center justify-center gap-1 shadow-lg shadow-emerald-600/20">
                 ...
                 <button onClick={() => processarEntrada('R')} className="flex-1 py-6 bg-red-600 hover:bg-red-500 text-white font-black rounded-xl transition-all flex flex-col items-center justify-center gap-1 shadow-lg shadow-red-600/20">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex flex-col items-center justify-center">
                ...
                <div className="h-48 w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={2}><Cell fill="#10b981" /><Cell fill="#ef4444" /></Pie></PieChart></ResponsiveContainer></div>
              ...
              <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-6 rounded-xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                   <div className="bg-black/30 p-4 rounded-xl border border-slate-800"><span className="text-[10px] text-slate-500 font-black uppercase block mb-1">Profit Factor</span><div className="text-xl font-black text-emerald-500">{stats.profitFactor}</div></div>
                   <div className="bg-black/30 p-4 rounded-xl border border-slate-800"><span className="text-[10px] text-slate-500 font-black uppercase block mb-1">Total Ciclos</span><div className="text-xl font-black text-blue-500">{stats.ciclosCompletos}</div></div>
                   <div className="bg-black/30 p-4 rounded-xl border border-slate-800"><span className="text-[10px] text-slate-500 font-black uppercase block mb-1">Total Entradas</span><div className="text-xl font-black text-white">{stats.totalEntradas}</div></div>
                   <div className="bg-black/30 p-4 rounded-xl border border-slate-800"><span className="text-[10px] text-slate-500 font-black uppercase block mb-1">Meta Batida</span><div className="text-xl font-black text-yellow-500">Sim</div></div>
                   <div className="bg-black/30 p-4 rounded-xl border border-slate-800"><span className="text-[10px] text-slate-500 font-black uppercase block mb-1">Média Stake</span><div className="text-xl font-black text-slate-300">R$ 0</div></div>
                   <div className="bg-black/30 p-4 rounded-xl border border-slate-800"><span className="text-[10px] text-slate-500 font-black uppercase block mb-1">Status Final</span><div className="text-xl font-black text-green-500">✓ ATIVO</div></div>
                </div>
              </div>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
               ...
               <div className="h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={historico.slice().reverse()}><CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} /><XAxis dataKey="data" stroke="#64748b" style={{ fontSize: '12px' }} /><YAxis stroke="#64748b" /><Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569' }} /><Line type="monotone" dataKey="bancaResultante" stroke="#3b82f6" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div>
            </div>
          </div>
        )}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-slate-950 text-slate-400 text-[10px] uppercase tracking-wider font-black"><tr><th className="px-6 py-4">Horário</th><th className="px-6 py-4">Stake</th><th className="px-6 py-4">Resultado</th><th className="px-6 py-4">Lucro/Prejuízo</th><th className="px-6 py-4">Saldo</th><th className="px-6 py-4">Status</th></tr></thead>
                <tbody className="divide-y divide-slate-800">
                  {historico.map((row) => (<tr key={row.id} className="hover:bg-slate-800/50 transition-colors"><td className="px-6 py-4 text-xs text-slate-400">{row.data}</td><td className="px-6 py-4 text-xs font-bold text-white">R$ {row.valor}</td><td className="px-6 py-4"><span className={`px-3 py-1 rounded-full text-[10px] font-black ${row.resultado === 'V' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-red-500/20 text-red-500'}`}>{row.resultado === 'V' ? '✓ VITÓRIA' : '✗ DERROTA'}</span></td><td className="px-6 py-4 text-xs font-bold"><span className={row.lucro.includes('-') ? 'text-red-500' : 'text-emerald-500'}>{row.lucro}</span></td><td className="px-6 py-4 text-xs font-bold text-white">R$ {row.bancaResultante}</td><td className="px-6 py-4 text-[10px] font-black text-blue-500 uppercase">{row.status}</td></tr>))}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {view === 'projecoes' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg">
              ...
              <div className="space-y-6">
                <div><label className="text-xs text-slate-500 uppercase font-bold mb-2 block tracking-widest">Banca Base (R$)</label><input type="number" value={config.bancaInicial} onChange={(e) => setConfig({ ...config, bancaInicial: parseFloat(e.target.value) || 0 })} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white font-bold focus:border-blue-500 outline-none transition-colors" /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-xs text-slate-500 uppercase font-bold mb-2 block text-emerald-500 tracking-widest">Stop Win (%)</label><input type="number" step="0.1" value={config.objetivoDiario} onChange={(e) => setConfig({ ...config, objetivoDiario: parseFloat(e.target.value) || 0 })} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white font-bold focus:border-emerald-500 outline-none transition-colors" /></div>
                  <div><label className="text-xs text-slate-500 uppercase font-bold mb-2 block text-red-500 tracking-widest">Stop Loss (%)</label><input type="number" step="0.1" value={config.stopLossDiario} onChange={(e) => setConfig({ ...config, stopLossDiario: parseFloat(e.target.value) || 0 })} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white font-bold focus:border-red-500 outline-none transition-colors" /></div>
                </div>
                <div className="pt-6 border-t border-slate-800">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button onClick={exportToCSV} disabled={historico.length === 0} className="flex items-center justify-center gap-2 py-3 px-4 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-lg transition-all text-xs uppercase tracking-widest disabled:opacity-50"><Download size={14} /> Exportar</button>
                    <button onClick={() => fileInputRef.current.click()} className="flex items-center justify-center gap-2 py-3 px-4 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-lg transition-all text-xs uppercase tracking-widest"><Upload size={14} /> Importar</button>
                  </div>
                </div>
                <button 
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-black/30 rounded-lg border-l-4 border-emerald-500"><span className="text-slate-400 text-[10px] font-black uppercase block mb-1">Estimativa 7 Dias</span><span className="text-emerald-500 text-lg font-black">R$ {(projecoes[7]?.valor || 0).toFixed(2)}</span></div>
                <div className="p-4 bg-black/30 rounded-lg border-l-4 border-blue-500"><span className="text-slate-400 text-[10px] font-black uppercase block mb-1">Estimativa 30 Dias</span><span className="text-blue-500 text-lg font-black">R$ {(projecoes[30]?.valor || 0).toFixed(2)}</span></div>
                <div className="p-4 bg-emerald-500/10 rounded-lg border border-emerald-500/20"><div className="flex justify-between items-center"><div><span className="text-emerald-500 font-black uppercase text-[10px] block">CRESCIMENTO ESTIMADO</span></div><span className="text-emerald-500 text-xl font-black">{((((projecoes[30]?.valor || 0) / bancaAtual) - 1) * 100).toFixed(1)}%</span></div></div>
              </div>
            </div>
          </div>
        )}
      ...
      <footer className="max-w-7xl mx-auto mt-12 mb-8 text-center"><div className="inline-block px-6 py-2 bg-slate-900 border border-slate-800 rounded-full"><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">© 2026 GESTÃO PRO • Sistema de Ciclos de Alavancagem</p></div></footer>
    </div>
  );
};
